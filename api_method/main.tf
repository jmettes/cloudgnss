...
resource "aws_api_gateway_integration" "request_method_integration" {
  ...
  type        = "AWS"
  uri         = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.region}:${data.aws_caller_identity.current.account_id}:function:${var.lambda}/invocations"
...
resource "aws_lambda_permission" "allow_api_gateway" {
  ...
  source_arn    = "arn:aws:execute-api:${var.region}:${data.aws_caller_identity.current.account_id}:${var.rest_api_id}/*/${var.method}${var.path}"
}
